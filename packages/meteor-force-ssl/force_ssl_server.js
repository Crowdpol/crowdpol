...
if (forceSSL !== false) {
  ...
  httpServer.addListener('request', function (req, res) {
    ...
    if ((forceSSL === true || !isLocal) && !isSsl) {
      ...
      /* LOCAL CHANGES */
      /* The request could come from any of our subdomains.
          We can not pre-emptively set the correct ROOT_URL as an environment variable
          So we get the subdomain from the req object instead, to make sure we have the right subdomain  */

      // Find the host the request came from instead of just forwarding to the root_url
